...
public class HttpUnitUtil {
    ...
    public static ArrayList<String> getStockTradeDetailFromDate(String _stockID, String _from_year, String _from_month,
            String _from_date,String _to_year,String _to_month,String _to_date) throws IOException, SAXException {
...
//        wc.setHeaderField(fieldName, fieldValue);
        ...
        if(_stockID.matches("00\\d{4}") || _stockID.matches("30\\d{4}")) {
            response = wc.getResponse("http://finance.yahoo.com/q/hp?s=" + _stockID + ".SZ&a=" + fromMonthOfYahoo + "&b="
                    + _from_date + "&c=" + _from_year + "&d="+toMonthOfYahoo+"&e="+_to_date+"&f=" + _to_year + "&g=d");
        } else {
        	 response = wc.getResponse("http://finance.yahoo.com/q/hp?s=" + _stockID + ".SS&a=" + fromMonthOfYahoo + "&b="
                     + _from_date + "&c=" + _from_year + "&d="+toMonthOfYahoo+"&e="+_to_date+"&f=" + _to_year + "&g=d");
        }
        ...
        if(table.length >= 4) {
            ...
            for(int i = 0; i < strlines.length; i++) {
                if(strlines[i].contains(_from_year)||strlines[i].contains(_to_year)) {
